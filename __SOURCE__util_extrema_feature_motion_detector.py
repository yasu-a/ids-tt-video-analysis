...
class ExtremaFeatureMotionDetector:
    def __init__(
            self,
            detection_region_rect: RectActualScaled,  # list of [height:slice, width:slice]
        ...
        assert isinstance(detection_region_rect, RectActualScaled), detection_region_rect

        self.__p_detection_region_rect = detection_region_rect
        self.__rect_indexer_with_color_channel = detection_region_rect.index3d
    ...
    def _process_input(self, img):
        return img[self.__rect_indexer_with_color_channel].astype(np.float32) / 256.0
    ...
    def _find_mutual_best_match(self, dist_mat):
        ...
        assert x.scale == y.scale, (x.shape, y.shape)
    ...
    def _extract_key_points(self, original_images, motion_images):
        ...
        def process():
            ...
            rect_offset = self.__rect_indexer_with_color_channel[0].start, \
                self.__rect_indexer_with_color_channel[1].start
        ...
        class ComputationResult(dict):
            ...
            def __get_accessor(self, suffix):
                class Accessor:
                    ...
                    frame: np.ndarray = ...
                    local_motion_center: np.ndarray = ...  # [[x, y], [x, y], ...]
